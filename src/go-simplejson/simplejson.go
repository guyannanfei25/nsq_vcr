...
// CopyAndSet create a copy of inner map, modifies the new `Json` map by `key` and `value`
// Useful for changing single key/value in a `Json` object, without changing json itself easily.
func (j *Json) CopyAndSet(key string, val interface{}) *Json {
	m, err := j.Map()
	if err != nil {
		return j
	}
